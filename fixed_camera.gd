...
func configure_camera(new_view_type: String, new_target: Node3D):
	...
	target = new_target
	orientation_node = target.get_child(1)  # Assuming the player has a child node for orientation
	...
	if target:
		...
		match view_type:
			"left":
				...
				var angle = -orientation_node.rotation.y
			...
			"right":
				...
				# Calculate position in a circle around the player
				var angle = -orientation_node.rotation.y -PI
				var pos_x = target.global_position.x + cos(angle) * offset.x
				var pos_z = target.global_position.z + sin(angle) * offset.x
				global_position = Vector3(pos_x, orientation_node.global_position.y + offset.y/3, pos_z)
